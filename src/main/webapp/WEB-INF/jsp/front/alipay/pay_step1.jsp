...
.progress-bar-default {
  ...
  <body>
                                ...
                                <div class="col-md-12 column">
                                    <table class="table table-bordered" style="text-align:center;">
                                      ...
                                      <tbody>
                                        <tr>
                                          <td>${project.name }</td>
                                          <td>${project.member.introduce}</td>
                                          <td>${ret.content }</td>
                                          <td style="color:#F60">￥${ret.supportmoney }</td>
                                         	<td> 
                                         		<c:if test="${ret.purchase==0 }">
                                         			<input type="text" value="1" onkeyup="changeMoney(${ret.supportmoney})" id="ordernum">
                                         		</c:if>
                                         		
                                         		<c:if test="${ret.purchase!=0 }">
                                         			限量${ret.purchase }  <input type="text" value="1"  placeholder="限量${ret.purchase }"
                                         			onkeyup="limitNum(${ret.purchase },this,${ret.supportmoney})" id="ordernum">
                                         		</c:if>
                                         	</td>
                                          <td>免运费</td>
                                        </tr>
                                    ...
                                    <p>总价(含运费)：￥<span style="font-size:16px;color:#F60;" id="money">${ret.supportmoney}</span></p>
                                    <button type="button" class="btn btn-warning btn-lg" style="float:right;"
                                     onclick="genOrder(${project.id},${ret.id })"></i> 去结算</button>
                                    </div>
                                </div>
	...
	<script>
		...
		function genOrder(projectid,retid){
			var money=$("#money").html();
			var ordernum=$("#ordernum").val();
			$.post(
				"insert_order",
				{"projectid":projectid,"retid":retid,"money":money,"ordernum":ordernum},
				function(data){
					if(data.code==0){
					...
					window.location.href="pay-step-2?retid="+data.code;
					
				}
			);
		}
		
		function changeMoney(submoney){
			var ordernum=$("#ordernum").val();
			$("#money").html(submoney*ordernum);
		}
		
		function limitNum(num,obj,submoney){
